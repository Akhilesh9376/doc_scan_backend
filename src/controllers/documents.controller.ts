...
import {
  generateAISummaryFromBuffer,
  generateAIInsightsFromBuffer,
  ...
  try {
    ...
    if (req.file.size > maxSize) {
      return res.status(400).json({
        success: false,
        message: isGuest
          ? `Guest users can only upload files up to 1MB (${(
              req.file.size /
              1024 /
              1024
            ).toFixed(2)}MB provided)`
    ...
    if (userId) {
      ...
      if (!isPremium && uploadCount >= FREE_TIER_DOCUMENT_LIMIT) {
        return res.status(429).json({
          ...
          error:
            "You have reached the free tier limit of 5 documents. Upgrade to premium for unlimited uploads.",
        });
      }
    }
  ...
  try {
    ...
    if (isMongoConnected()) {
      try {
        ...
        documents = Array.from(documentsMap.values()).filter((d) =>
...
// ---- Analyze Document ----
export const handleAnalyzeDocument: RequestHandler = async (req, res) => {
  const DocumentModel = getModel("Document");
  ...
  try {
    ...
    if (!process.env.GOOGLE_GEMINI_API_KEY) {
      ...
      console.error(
        "[ANALYSIS] Document buffer missing in memory. Falling back to mock."
      );
      summary = generateMockSummary(document.name);
      insights = generateMockInsights(document.name);
    } else {
      try {
        console.log(
          "[ANALYZE] Generating AI summary and insights for document:",
          document.name
        );
        summary = await generateAISummaryFromBuffer(
          buffer,
          document.type,
          document.name
        );
        insights = await generateAIInsightsFromBuffer(
          buffer,
          document.type,
          document.name
        );
        ...
        console.error(
          "[ANALYSIS] AI generation failed, falling back to mock data:",
          err
        );
        summary = generateMockSummary(document.name);
        insights = generateMockInsights(document.name);
    ...
    if (isMongoConnected()) {
      try {
        ...
        await DocumentModel.findOneAndUpdate(
          { documentId },
          { analyzed: true, analysisStatus: "completed" }
        );
      } catch {
